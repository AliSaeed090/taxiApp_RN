...
import { onChangeTheme,onChangeFont, onForceTheme,onChangeLanguage } from "../../redux/slices/application";
...
export default function ThemeSetting({ navigation }) {
   ...
   *
   */
  const onChangeThemeFunction = () => {
    const list = themeSupport.filter((item) => item.selected);
    if (list.length > 0) {
      dispatch(onChangeTheme(list[0].theme));
  ...
  return (
    <SafeAreaView style={BaseStyle.safeAreaView} edges={['right', 'top', 'left']}>
      ...
      <View style={{ paddingHorizontal: 20, paddingVertical: 15 }}>
        <Button full onPress={onChangeThemeFunction}>
          {t("apply")}
        </Button>
      </View>
    </SafeAreaView>
