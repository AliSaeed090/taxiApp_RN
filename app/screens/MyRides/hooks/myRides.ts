...
const fetchMyRides = async ({ pageParam = null }: { pageParam?: FirebaseFirestoreTypes.QueryDocumentSnapshot<Ride> | null }): Promise<FetchMyRidesResponse> => {
  ...
  let query = firestore()
    ...
    .limit(10).where('userId', '==', user.uid); // Number of documents per page
